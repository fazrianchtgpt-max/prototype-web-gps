...
app.get('/', (req, res) => res.send('GPS SERVER IS RUNNING ≡ƒÜÇ'));
...
// ============================================================
// COMMAND PACKET GT06N ΓÇö Format Standard 7878
// ============================================================
...
const gpsServer = net.createServer((socket) => {
        ...
        try {
            ...
            while (tcpBuffer.length >= 4) {
                ...
                if (tcpBuffer[0] === 0x78 && tcpBuffer[1] === 0x78) {
                    ...
                    console.log(`[WARN] Invalid byte 0x${tcpBuffer[0].toString(16)} ΓÇö discarding`);
    ...
    socket.on('error', (err) => {
        console.log(`[${new Date().toLocaleTimeString()}] ΓÜá∩╕Å Socket Error (${currentImei || 'unknown'}): ${err.message}`);
    });
    ...
    socket.on('close', () => {
        if (currentImei) {
            ...
            console.log(`[${new Date().toLocaleTimeString()}] ≡ƒö┤ Disconnect: ${currentImei}`);
        }
    });
    ...
    function processPacket(socket, data) {
        ...
        console.log(`[${new Date().toLocaleTimeString()}] ≡ƒôª [${packetId}] ${isExtended ? 'EXT' : 'STD'} len=${data.length} | ${hex.substring(0, 50)}`);

        // ΓöÇΓöÇΓöÇ 1. LOGIN (0x01) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        if (isStandard && packetId === '01') {
            ...
            console.log(`[${new Date().toLocaleTimeString()}] ≡ƒƒó Login: ${currentImei}`);
        }

        // ΓöÇΓöÇΓöÇ 2. HEARTBEAT / STATUS (0x13, 0x26, 0x94) ΓöÇΓöÇΓöÇΓöÇΓöÇ
            ...
            if (currentImei) {
                ...
                if (pending && pending.targetStatus === newRelay) {
                    ...
                    clearPendingRelay(currentImei);
                    console.log(`[${new Date().toLocaleTimeString()}] Γ£à Relay confirmed via heartbeat: ${currentImei} ΓåÆ ${newRelay}`);
                }
                ...
                console.log(`[${new Date().toLocaleTimeString()}] ΓÜí STATUS: ${currentImei} | ACC: ${newAcc} | Mesin: ${newRelay} | infoByte: 0x${infoByte.toString(16).padStart(2, '0')}`);
            }
        }

        // ΓöÇΓöÇΓöÇ 3. LOCATION (0x12, 0x22) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            ...
            try {
                ...
                console.log(`[${new Date().toLocaleTimeString()}] ≡ƒôì LOC: ${currentImei} | lat:${payload.lat} lon:${payload.lon} spd:${speed}`);
            } catch (e) {
                console.error(`[Location Parse Error] ${e.message}`);
            }
        }

        // ΓöÇΓöÇΓöÇ 4. COMMAND RESPONSE dari alat (0x15) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        else if (packetId === '15') {
            console.log(`[${new Date().toLocaleTimeString()}] ≡ƒôÑ CMD Response RAW: ${hex}`);
            ...
            try {
                ...
                if (responseText) {
                    console.log(`[${new Date().toLocaleTimeString()}] ≡ƒô¿ Response Text: "${responseText}"`);
                    if (currentImei) {
                        ...
                        if (responseText.toUpperCase().includes('RELAY')) {
                            ...
                            if (pending) {
                                ...
                                clearPendingRelay(currentImei);
                                console.log(`[${new Date().toLocaleTimeString()}] Γ£à Relay confirmed via 0x15: ${currentImei} ΓåÆ ${confirmedRelay}`);
                            }
                        }
                    }
                }
        ...
        // ΓöÇΓöÇΓöÇ 5. ALARM (0x16, 0x19, 0x98, 0x2C) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            ...
            console.log(`[${new Date().toLocaleTimeString()}] ≡ƒÜ¿ ALARM [${packetId}]: ${hex}`);
        }

        // ΓöÇΓöÇΓöÇ Unknown packet ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        else {
            console.log(`[${new Date().toLocaleTimeString()}] Γ¥ô Unknown [${packetId}]: ${hex}`);
        }
    }
});

// ============================================================
// WEBSOCKET ΓÇö CLIENT COMMANDS
// ============================================================
io.on('connection', (ws) => {
    console.log(`[${new Date().toLocaleTimeString()}] ≡ƒîÉ Client Connected: ${ws.id}`);
    ...
    ws.on('send_command', (d) => {
        ...
        try {
            ...
            s.write(packet, (err) => {
                ...
                console.log(`[${new Date().toLocaleTimeString()}] ≡ƒöî Sent [${d.command}] ΓåÆ ${d.imei}`);
                ...
                if (d.command.toUpperCase().includes('RELAY')) {
                    ...
                    const timer = setTimeout(() => {
                        if (pendingRelayConfirmations[d.imei]) {
                            ...
                            console.log(`[${new Date().toLocaleTimeString()}] ΓÅ░ Relay timeout: ${d.imei}`);
                        }
                    }, 30000);
    ...
    ws.on('disconnect', () => {
        console.log(`[${new Date().toLocaleTimeString()}] ≡ƒîÉ Client Disconnected: ${ws.id}`);
...
gpsServer.listen(GPS_PORT, '0.0.0.0', () => {
    console.log(`≡ƒÜÇ GPS TCP Server listening on port ${GPS_PORT}`);
});

webServer.listen(WEB_PORT, '0.0.0.0', () => {
    console.log(`≡ƒîÉ Web/WS Server listening on port ${WEB_PORT}`);
});
